...
(async function () {
  try {
    const postqus = await fetch("http://localhost:3000/api/level1");
    FirstQus = await postqus.json();

    const postans = await fetch("http://localhost:3000/api/level1");
...
correct1.addEventListener("click", async function () {
  if ((Div1.style.display = "block")) {
    ...
    try {
      ...
      await fetch("http://localhost:3000/api/level1", { method: "DELETE" });

      // Fetch and display the "http://localhost:8080/api/level2" data
      const response = await fetch("http://localhost:3000/api/level2");
  ...
  correct2.addEventListener("click", async function () {
    if ((Div2.style.display = "block")) {
      ...
      try {
        ...
        await fetch("http://localhost:3000/api/level2", { method: "DELETE" });

        // Fetch and display the "http://localhost:8080/api/level3" data
        const response = await fetch("http://localhost:3000/api/level3");
    ...
    correct3.addEventListener("click", async function () {
      if ((Div3.style.display = "block")) {
        ...
        try {
          ...
          await fetch("http://localhost:3000/api/level3", { method: "DELETE" });

          // Fetch and display the "http://localhost:8080/api/level4" data
          const response = await fetch("http://localhost:3000/api/level4");
      ...
      correct4.addEventListener("click", async function () {
        if ((Div4.style.display = "block")) {
          ...
          try {
            ...
            await fetch("http://localhost:3000/api/level4", {
              method: "DELETE",
            });

            // Fetch and display the "http://localhost:8080/api/level5" data
            const response = await fetch("http://localhost:3000/api/level5");
